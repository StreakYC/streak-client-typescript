...
```sh
npm i -s @streakyc/streak
```

## Reference

A full reference for this library is available [here](https://github.com/StreakYC/streak-client-typescript/blob/HEAD/./reference.md).
...
const request: Streak.GetV1ApikeysRequest = {
    ...
};
```
...
```typescript
import { StreakError } from "@streakyc/streak";

try {
    await client.pipeline.updatePipelineField(...);
} catch (err) {
    if (err instanceof StreakError) {
...
```typescript
import { ApiKeysClient } from '@streakyc/streak/apiKeys';
...
```typescript
const { data, rawResponse } = await client.pipeline.updatePipelineField(...).withRawResponse();
...
```typescript
import { StreakClient, logging } from "@streakyc/streak";
