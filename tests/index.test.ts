...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Streak({ logger: logger, logLevel: 'debug', apiKey: 'My API Key', foo: 'My Foo' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Streak({ apiKey: 'My API Key', foo: 'My Foo' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Streak({ logger: logger, logLevel: 'info', apiKey: 'My API Key', foo: 'My Foo' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Streak({ logger: logger, apiKey: 'My API Key', foo: 'My Foo' });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['STREAK_LOG'] = 'debug';
      const client = new Streak({ logger: logger, logLevel: 'off', apiKey: 'My API Key', foo: 'My Foo' });
  ...
  describe('baseUrl', () => {
    ...
    test('explicit option', () => {
      const client = new Streak({ baseURL: 'https://example.com', apiKey: 'My API Key', foo: 'My Foo' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Streak({ maxRetries: 4, apiKey: 'My API Key', foo: 'My Foo' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Streak({ apiKey: 'My API Key', foo: 'My Foo' });
  ...
  test('with environment variable arguments', () => {
    ...
    const client = new Streak({ foo: 'My Foo' });
    expect(client.apiKey).toBe('My API Key');
    expect(client.foo).toBe('My Foo');
  });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Streak({ apiKey: 'My API Key', foo: 'My Foo', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Streak({ apiKey: 'My API Key', foo: 'My Foo', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Streak({ apiKey: 'My API Key', foo: 'My Foo', fetch: testFetch });
